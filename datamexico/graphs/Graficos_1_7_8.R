...
# Grafico de dispersion. Participacion vs. Letalidad ----------------------

base <- letalidad_estado %>% 
  left_join(participaciones)


ggplot(base, aes(x= letalidad, y = pps, label = state)) +
  geom_point(aes(x= letalidad, y = pps), color = "#457b9d") +
  geom_text(hjust=0, vjust=-0.5, size = 2) +
  theme_minimal()+  
  #geom_vline(xintercept = 0.0307) + geom_hline(yintercept = 721184) +
  labs(
    title = "Participaciones estatales y letalidad ante el COVID",
    x= "Letalidad",
    y = "Participaciones por 100 habitantes"
  ) +
  geom_smooth(method = "lm", se = F, color= "#ffba49")
